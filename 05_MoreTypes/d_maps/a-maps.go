...
func main() {
	...
	fmt.Println("reflect.TypeOf(alphabets)       =", reflect.TypeOf(alphabets))        // map[string]string
	fmt.Println("reflect.TypeOf(alphabets).Kind()=", reflect.TypeOf(alphabets).Kind()) // map
	fmt.Println()

	another := map[int]bool{
		123:  true,
		-234: false,
		34:   true,
		5:    true,
	...
	fmt.Println("another=", another)            // map[-234:false 5:true 34:true 123:true]
	fmt.Println("len(another) =", len(another)) // 4
	// fmt.Println("cap(another) =", cap(another)) //  doesnt work

	someThing := map[bool]string{
		true:  "This is True",
		false: "This is False",
		// true: "this is again true",  //  duplicate key true in map literal
	}
	fmt.Println("someThing = ", someThing)          // map[false:This is False true:This is True]
	fmt.Println("len(someThing) =", len(someThing)) // 2
	...
	fmt.Println("timeZone = ", timeZone) //  map[CST:-21600 EST:-18000 MST:-25200 PST:-28800 UTC:0]
