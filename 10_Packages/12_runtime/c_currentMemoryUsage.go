...
func PrintMemUsage() {
	...
	// Alloc - Amount allocated heap objects
	// Alloc - memory is not reduced necessarily when a variable is cleared, but when garbage collector runs.
	fmt.Printf("Alloc = %2v MiB", bToMb(m.Alloc))
	fmt.Printf("\tTotalAlloc = %2v MiB", bToMb(m.TotalAlloc))
	fmt.Printf("\tSys = %2v MiB", bToMb(m.Sys))
	fmt.Printf("\tNumGC = %2v\n", m.NumGC)
}
